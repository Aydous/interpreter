...
string follow[200];
void read() {
	memset(table,-1,sizeof(table));//初始化
	getnum['#'] = 0;
	c[0] = '#';
	cout << "请输入终结符集" << endl;
	char x;
	...
	cout << "请输入所有产生式（空字用@表示），以end结束：" << endl;
	string pro;
...
void jiao(string &a, string b) {//取a和b的交集，并赋值给a
	set<char>se;
...
void getfirst() {
	...
	for (unsigned int j = 0;j< proce.size();j++) {//扫描所有产生式
		int k = 0;int has_0 = 0;//k 扫描该产生式
		do {
			has_0 = 0;
			k++;
			if (k == proce[j].size()) {//推到最后一个，则附加空字
				first[getnum[proce[j][0]]] += ('0' + numvt);
				break;
			}
			jiao(first[getnum[proce[j][0]]], get_f(getnum[proce[j][k]], has_0));//合并
		} while (has_0);//直到无法推出空字为止
	}
}
void print_first() {
	cout << "first集如下：" << endl;
...
void getfollow() {
	jiao(follow[getnum[proce[0][0]]], "0");//先添加‘#’
	for (unsigned int j = 0;j< proce.size();j++) {//扫描产生式
		for (unsigned int jj = 1;jj < proce[j].size();jj++) {//每个非终结符的follow集
			...
			do
			{
				has_0 = 0;
				k++;
				if (k == proce[j].size()) {//都能推出空字，follow集等于产生式左边的vn
...
string get_proce(int i) {//由对应下标获得产生式
	if (i < 0)return "";
	string ans;
	ans += proce[i][0];
	ans += "->";
...
string word;
bool analyze() {//总控，分析word的合法性，若合法，输出所有产生式
	stack<char>sta;
	sta.push('#');
	sta.push(proce[0][0]);
	int i = 0;
...
int main() {
	...
	cin >> word;
	if (analyze()) {
		cout << "succeed!该字有效，所有产生式如上。" << endl;
	}
	else {
		cout << "error!" << endl;
	}
